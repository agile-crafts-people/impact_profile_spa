<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Platforms</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="platform-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/platforms/new"
          data-automation-id="platform-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(platforms ?? []) as unknown as Platform[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToPlatform"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="platform-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load platforms: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Platforms List Page - Showcase of impact_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Platform>({
  queryKey: ['platforms'],
  queryFn: (params) => api.getPlatforms(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToPlatform(_event: unknown, { item }: { item: Platform }) {
  router.push(`/platforms/${item._id}`)
}
